...
function collectFromBattlefield() {
  ...
  battlefieldTimeout = setTimeout(() => {
    ...
    if (newCount > 0) {
      console.log(`[DataCentre] Captured ${newCount} new players`);
    }
...
async function collectTIVFromAttackPage() {
  ...
  // === Push to API ===
  await sendToAPI("tiv", { playerId: id, tiv });
}
...
async function collectTIVAndStatsFromArmory() {
  ...
  if (tiv) {
    ...
    // 🔥 Send TIV to API
    await sendToAPI("tiv", { playerId: myId, tiv });
  }
...
async function collectFromReconPage() {
  ...
  if (stats.tiv) {
    await sendToAPI("tiv", { playerId: id, tiv: stats.tiv });
  }
